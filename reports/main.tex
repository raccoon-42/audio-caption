...
\title{ClapCap: Lightweight Language Models for Music Captioning via a CLAP-to-LM Prefix Pipeline}

\oneauthor{Ali Ozkaya}{Izmir Institute of Technology \\ {\tt aliozkaya00@gmail.com}}
...
improving semantics; and (iv) qualitatively the models capture genre, instrumentation and mood
well but share a systematic bias toward hallucinating a vocalist. We frame the work as an
...
\begin{itemize}\itemsep2pt
  ...
  \item An honest qualitative analysis, including a reproducible vocalist-hallucination failure
        mode that we trace to the general-purpose audio frontend.
\end{itemize}

We state up front what this paper is \emph{not}: it is not a state-of-the-art system, and we do
not compare our numbers against systems evaluated on different datasets
(Section~\ref{sec:related}). The value is the controlled analysis and the efficiency of the
recipe --- only a small projection (plus the LM) is trained.
...
recover, amplified, when scaling the LM (Section~\ref{sec:limitations}).
...
AudioSet, each annotated by musicians with a multi-sentence caption and a list of musical
...
\textbf{Audio encoder.}
We use the LAION \clapenc{} checkpoint \texttt{laion/clap-htsat-unfused}~\cite{wu2023clap}, a
$512$-dimensional audio embedding. \clapenc{} is trained on general audio and is \emph{not}
...
\]
We use AdamW (weight decay $0.1$), cosine annealing, batch size $8$ and early stopping
(patience $5$). Per model, only the learning rates are tuned, via a 16-trial Optuna search over
an 8-epoch proxy; all else is fixed so the LM stays the controlled variable.
...
billion-parameter end-to-end audio LLMs --- and a full two-stage run trains in
\textbf{[TODO: $\sim$X\,h]} on a single NVIDIA RTX~5090. Because \clapenc{} embeddings are
precomputed once, the audio encoder is never run during training.
...
\textbf{Metrics.}
A single shared harness scores all conditions with the \texttt{aac-metrics} suite: BLEU-1,
BLEU-4, METEOR, ROUGE-L, CIDEr-D, SPICE, SPIDEr, SBERT-sim, FER and \fense{}.
\fense{}~\cite{zhou2022fense} is our primary metric: it combines a sentence-BERT semantic
similarity with a fluency-error penalty (FER), making it sensitive to the semantic correctness a
listener cares about. We apply no post-processing by default; sentence trimming is treated as an
...
\input{tables/paper_baseline.tex}
...
(CIDEr-D, SPIDEr in Table~\ref{tab:decoding}) barely move. In other words, trimming removes the
...
\input{tables/paper_decoding.tex}
...
occasionally degenerates into verbatim repetition. Most strikingly, both models default to
describing ``a male vocalist singing melodically'' even when the reference is instrumental or
explicitly states there is no singer. We attribute this vocal-presence blind spot to the
general-purpose audio frontend and the strong language prior of the caption template.
...
music-specialised encoder (a music \clapenc{} checkpoint, MERT or MusicFM) is a natural and
likely beneficial swap, especially for the fine-grained attributes (vocal presence, instrument
identity) where the model is weakest.
...
rather than by improving semantics. A reproducible vocalist hallucination points to the
general-purpose audio frontend as the next thing to improve. The results characterise the
minimum viable architecture for music captioning and the care needed to interpret its metrics,
rather than chasing state of the art.
