...
class Projection(nn.Module):
    def __init__(self, audio_dim, lm_dim, prefix_len, dropout=0.3, depth=2):
        super().__init__()
        ...
        if depth == 1:
            ...
            layers = []
            layers += [nn.Linear(audio_dim, lm_dim * 2), nn.GELU(), nn.Dropout(dropout)]
            for _ in range(depth - 2):
                layers += [nn.Linear(lm_dim * 2, lm_dim * 2), nn.GELU(), nn.Dropout(dropout)]
