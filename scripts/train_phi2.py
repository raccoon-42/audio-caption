"""Self-contained Phi-2 audio captioning training script (stage 1 + stage 2)."""
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/phi2.yaml")
    ...
    tag = args.ablation_tag or "phi2"
    ckpt_dir = Path(cfg["checkpoint_dir"]) / tag
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    results_dir = Path(cfg["results_dir"]) / "phi2"
    ...
    # ========== STAGE 2: projection + Phi-2 fine-tune ==========
    ...
    best_val, history_s2 = train_loop(
        ...
        ckpt_paths={
            ...
            "lm": (model.state_dict, ckpt_dir / "stage2_phi2_best.pt"),
        },
        device=device,
        t_max=20,
    )
    torch.save(projection.state_dict(), ckpt_dir / "stage2_proj_last.pt")
    torch.save(model.state_dict(), ckpt_dir / "stage2_phi2_last.pt")
