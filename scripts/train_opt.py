...
def main():
    ...
    for epoch in range(1, s1["epochs"] + 1):
        train_loss = train_one_epoch(
            projection, model, train_loader, optimizer, prefix_len, device
        )
        val_loss = evaluate(projection, model, val_loader, prefix_len, device)
    ...
    for epoch in range(1, s2["epochs"] + 1):
        ...
        if val_loss < best_val:
            ...
            torch.save(model.state_dict(), ckpt_dir / "stage2_opt_best.pt")
